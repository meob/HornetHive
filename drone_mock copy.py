        ...
        if HOME and status != "STASIS":
            ...
            if mission_type == "RTL" and target["z"] > 0:
                if dist_to_center < 5.0:
                    ...
                    current_target_z = pos["z"]

            dz = current_target_z - pos["z"]
            if abs(dz) > 0.1:
                pos["z"] += math.copysign(min(abs(dz), CONFIG["climb_rate"] * dt), dz)
            ...
            if pos["z"] > 0.5:
                if CONFIG["type"] == "FIXED_WING":
                    ...
                    if (dist_to_center < orbit_radius + 50.0 or is_loitering or is_rtl_descending):
                        if not is_loitering:
                            is_loitering = True
                    ...
                    if dist_to_center > 2.0: 
                        ...
                        if mission_type in ["STRIKE", "SENSOR", "RESCUE_TUBE", "MEDKIT"]:
                            ...
                            client.publish("hive/alerts/detection", json.dumps({"sensor": ID, "event": event_type, "lat": pos["lat"], "lon": pos["lon"], "ts": int(now)}))
                            client.publish("hive/alerts/action_performed", json.dumps({"drone_id": ID, "capability": CAPABILITY, "event": event_type, "lat": pos["lat"], "lon": pos["lon"], "x": pos["x"], "y": pos["y"], "ts": int(now)}))
                            log_event(f"ACTION EXECUTED: {event_type} ({CAPABILITY})")
                            if CONFIG["behavior_type"] == "KAMIKAZE": 
                                log_event("Kamikaze impact. Terminating process.")
                                sys.exit()
                            else:
                                if CAPABILITY == "STRIKE": mission_type, target["z"] = "RTL", 120
                                else: mission_type, status = "IDLE", "HOVERING"
                        else:
                            status = "HOVERING"
